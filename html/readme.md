...
4. <code>figure</code> 태그 : 들여쓰기가 돼. 하나의 그림 처럼 설명 해. 독립적인 콘텐츠를 표현한데 : 보통 위나 아래의 설명, 콘텐츠의 하나의 단위로 참조된데 (아예 사용하지않아도 뭐 상관 없음 - div대체) <code>figcaption</code>을 통해 해당 figure을 설명줄 수 있어
...
<code>target</code>속성 -> 링크한 URL을 표시할 위치. 가능한 값은 브라우징 맥락으로, 즉 탭, 창, iframe의 이름이나 특정 키워드입니다. _blank 값을 많이 쓴다는데 default로 _self    👉 ```<a href="/www.naver.com" target="_blank">네이버</a>``` , _self, _parent, _top, 
...
3. ```details``` 태그는 이 태그를 클릭했을때(onClick 시) 안에 내용이 나와
```
<details>
    <summary>대표로 보여지는 내용</summary>
    <p>눌렀을 때 보여지는 내용</p>
</details>
```
4. ```meter```태그와 ```progress``` 태그 : input으로도할 수 있는데, value값으로 얼만큼 찼는지, max값으로 몇간으로 나눴는지 각각 1.0 / 10을 줘